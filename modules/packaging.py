...
# ---------------- ROLE CHECK ----------------
# Allow both packing and dispatch roles
        ...
        for key, o in sorted_packing:
            ...
            with st.container(border=True):
                ...
                with col_time_files:
                    ...
                    if not start:
                        if st.button("▶️ Start Packing", key=f"p_start_{order_id}", use_container_width=True, type="secondary"):
                            update(f"orders/{key}", {"packing_start": datetime.now(timezone.utc).isoformat()}) 
                            st.rerun()
                    elif not end:
                        if st.button("⏹ End Packing", key=f"p_end_{order_id}", use_container_width=True, type="primary"):
                            update(f"orders/{key}", {"packing_end": datetime.now(timezone.utc).isoformat()}) 
                            st.rerun()
                    ...
                    slip_details = {"assign": current_assign, "material": current_material, "notes": current_notes}
                ...
                with col_details:
                    ...
                    assign = st.text_input("Assign To", current_assign, key=f"p_assign_{order_id}")
                ...
                with col_time_files:
                    ...
                    if not start:
                        if st.button("▶️ Start Dispatch", key=f"d_start_{order_id}", use_container_width=True, type="secondary"):
                            update(f"orders/{key}", {"dispatch_start": datetime.now(timezone.utc).isoformat()}) 
                            st.rerun()
                    elif not end:
                        if st.button("⏹ End Dispatch", key=f"d_end_{order_id}", use_container_width=True, type="primary"):
                            update(f"orders/{key}", {"dispatch_end": datetime.now(timezone.utc).isoformat()}) 
                            st.rerun()
                        start_dt = parse_datetime_robust(start)
                        st.info(f"Running since: **{start_dt.astimezone(timezone(timedelta(hours=5, minutes=30))).strftime('%Y-%m-%d %H:%M IST')}**" if start_dt else "Running...")
                    else:
                    ...
                    slip_details = {"courier": current_courier, "tracking": current_tracking, "notes": current_notes}
    ...
    for key, o in sorted_completed_final:
        ...
        with st.expander(f"✅ {o['order_id']} — {o.get('customer')} | Finalized: {comp_dt_ist.split(' ')[0]}"):
            ...
            with col_dispatch:
                ...
                - **Dispatched By:** `{o.get("dispatch_assigned", "N/A")}`
                """)
