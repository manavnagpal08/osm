...
# DOWNLOAD HELPER
# ========================================================
def download_button_ui(b64_data, filename, label, key):
    if not b64_data:
        return
...
def file_card(col, order_id, file_key, label, allowed, firebase_key):
    ...
    existing_entry = design_files.get(file_key, {})

    file_b64 = existing_entry.get("data")
    file_ext = existing_entry.get("ext")
    ...
    with col:
        ...
        # Download
        if file_b64 and file_ext:
            filename = f"{order_id}_{file_key}.{file_ext}"
