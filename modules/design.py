    ...
    for firebase_key, order in pending_orders.items():
        ...
        with st.expander(f"**{order_id}** — {order.get('customer')} | Item: {order.get('item')}", expanded=False):
            ...
            with fcol:
                ...
                # Updated allowed types to remove 'ai' and 'zip', and include common image formats
                file_card(f2, order_id, "template", "Template/Mockup", ["pdf", "png", "jpg", "jpeg"], firebase_key)
                # Updated allowed types to remove 'ai' and 'zip', and include common image formats
                file_card(f3, order_id, "final", "Final Art", ["pdf", "png", "jpg", "jpeg"], firebase_key)
